document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 401) {
                container.innerHTML = "<p style='grid-column:1/-1; text-align:center; margin-top:50px;'>Musisz być zalogowany.</p>";
                return;
            }
...
async function loadStats() {
    try {
        ...
        if(res.ok) {
            ...
            // Nowa statystyka
            document.getElementById("stat-runtime").textContent = stats.avg_runtime;
        }
...
function renderFavorites(movies) {
    ...
    if (!movies || movies.length === 0) {
        container.innerHTML = "<p style='grid-column: 1/-1; text-align:center; color:#888; font-size:1.2rem; margin-top:50px;'>Lista jest pusta.</p>";
        return;
    }
    container.innerHTML = "";
            ...
            try {
                ...
                if(res.ok) {
                    ...
                    if(data.removed) {
                        ...
                        setTimeout(() => {
                            card.remove();
                            if(container.children.length === 0) container.innerHTML = "<p style='grid-column: 1/-1; text-align:center; color:#888; margin-top:50px;'>Lista jest pusta.</p>";
...
function createFavCard(movie) {
    ...
    const posterUrl = movie.poster_path ? `https://image.tmdb.org/t/p/w300${movie.poster_path}` : "https://via.placeholder.com/200x300?text=Brak+Okładki";
    ...
    div.innerHTML = `
        ...
        <div class="movie-rating-box">
            ...
            <div class="rating-bar"><div class="rating-fill" style="width: ${ratingPercent}%;"></div></div>
        </div>
